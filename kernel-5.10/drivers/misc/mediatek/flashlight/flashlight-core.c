		...
		const int type, const int ct, const int part)
{
	...
	list_for_each_entry(fdev, &flashlight_list, node) {
		pr_info("flashlight_find_dev_by_full_index fdev->dev_id.type=%d,type=%d",fdev->dev_id.type,type);
	    pr_info("flashlight_find_dev_by_full_index fdev->dev_id.ct=%d,ct=%d",fdev->dev_id.ct,ct);
	    pr_info("flashlight_find_dev_by_full_index fdev->dev_id.part=%d,part=%d",fdev->dev_id.part,part);
		...
		const int type, const int ct)
{
	...
	/* return the first flashlight device */
	list_for_each_entry(fdev, &flashlight_list, node) {
		pr_info("flashlight_find_dev_by_index(remove) fdev->dev_id.type=%d,type=%d",fdev->dev_id.type,type);
	    pr_info("flashlight_find_dev_by_index fdev->dev_id.ct=%d,ct=%d",fdev->dev_id.ct,ct);
		...
		struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_info("_flashlight_ioctl start\n");
...
static void hs07_create_torch_node(void)
{
    hs07_torch_class = class_create(THIS_MODULE,"camera");
