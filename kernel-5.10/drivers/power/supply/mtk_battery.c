...
static enum power_supply_property battery_props[] = {
	...
	/* HS07 code for  SR-AL7761A-01-79 by yexuedong at 20250317 start */
	/* HST11 code for AX7800A-515 by wenyaqi at 20250421 start */
	#if defined(CONFIG_CUSTOM_PROJECT_HS07) || defined(CONFIG_CUSTOM_PROJECT_HST11)
	/* HST11 code for AX7800A-515 by wenyaqi at 20250421 end */
	POWER_SUPPLY_PROP_VOLTAGE_OCV,
	#endif
	/* HS07 code for  SR-AL7761A-01-79 by yexuedong at 20250317 end */
};
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_OCV:
		ret = gauge_get_property(GAUGE_PROP_BOOT_ZCV, &volt_ocv);
		if(ret < 0) {
			bm_err("get voltage_ocv property fail\n");
			val->intval = -1;
		} else {
			val->intval = volt_ocv * 100;
		}
		break;
	#endif
	/* HS07 code for  SR-AL7761A-01-79 by yexuedong at 20250317 end */
...
void battery_update(struct mtk_battery *gm)
{
	...
	/* HST11 code for P250701-05046 by zhangziyi at 20250703 start */
		...
		bat_data->bat_status = POWER_SUPPLY_STATUS_FULL;
		bm_err("In battery protect and battery_update set FULL! ui:%d status:%d\n",
			bat_data->bat_capacity, bat_data->bat_status);
	}
	#endif//CONFIG_ODM_CUSTOM_FACTORY_BUILD
