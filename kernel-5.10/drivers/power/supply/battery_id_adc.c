...
/* HST11 code for AX7800A-616 by wenyaqi at 20250429 start */
#if defined(CONFIG_CUSTOM_PROJECT_HST11)
static bool g_support_auth_ic = false;
...
static bool auth_area_pcbainfo_judge(void)
{
    ...
    pr_info("[%d, %d]", g_board_id_status, board_id);
...
#endif
/* HST11 code for AX7800A-616 by wenyaqi at 20250429 end */
...
int battery_get_profile_id(void)
{
    ...
    if (g_support_auth_ic && auth_area_pcbainfo_judge()) {
        // not achieve auth_logic yet, use default value
        id = GXY_BATTERY_TYPE_AUTH_ATL;
    } else {
    ...
    pr_err("[%s] bat_id_get_adc_num:%d, id_volt:%d, bat_id:%d, auth:%d\n",
        __func__, bat_id_get_adc_num(), id_volt, id, g_support_auth_ic);
    return id;
}
...
int bat_id_adc_probe(struct platform_device *pdev)
{
    ...
    /* HST11 code for AX7800A-616 by wenyaqi at 20250425 start */
    #if defined(CONFIG_CUSTOM_PROJECT_HST11)
    g_support_auth_ic = of_property_read_bool(node, "support_auth_ic");
    #endif
    /* HST11 code for AX7800A-616 by wenyaqi at 20250425 end */
