...
int gcore_upgate_reset(void)
{
    int ret = 0;
    u8 cmd_1[] = {0x04,0xFB,0xC0,0x00,0x00,0x90};
    //u8 cmd_2[] = {0x64,0x00,0x10,0x00};
    u8 cmd_read2[4] ={0};
    u8 cmd_2[] = {0x05,0xFA,0xC0,0x00,0x00,0x90,0x01,0x00,0x10,0x00};
    u8 cmd1[7] = { 0x05, 0xFA, 0x40, 0x00, 0x00, 0x50, 0x00};
    u8 cmd2[10] = {0x05, 0xFA, 0x80, 0x02, 0x00, 0x00, 0x00,0x01,0x00,0x00};
    u8 cmd3[23] = {0x05, 0xFA, 0x80, 0x02, 0x01, 0x00, 0x02, 0xEA,0x00, 0x40, 0x23,0xEA,0x00,0x40,0x00,0x30,0x54,0xB3,0x3C,
        0x78,0x00,0x00};
    u8 cmd4[7] = {0x05, 0xFA, 0x40, 0x00, 0x00, 0x50, 0x01};
    gcore_enter_idm_mode();
    usleep_range(3000, 3100);
    ret = gcore_bus_write(cmd_1,sizeof(cmd_1));
    if (ret) {
        GTP_ERROR("write cmd_1 fail");
        return -EPERM;
    }
    usleep_range(1000, 1100);
    ret = gcore_bus_read(cmd_read2,sizeof(cmd_read2));
    GTP_DEBUG("read cmd_read2 = %d,%d,%d,%d",cmd_read2[0],cmd_read2[1],cmd_read2[2],cmd_read2[3]);
    usleep_range(3000, 3100);
    ret = gcore_bus_write(cmd_2,sizeof(cmd_2));
    if (ret) {
        GTP_ERROR("write cmd_2 fail");
        return -EPERM;
    }
    usleep_range(3000, 3100);
    ret = gcore_bus_write(cmd1,sizeof(cmd1));
    if (ret) {
        GTP_ERROR("write cmd1 fail");
        return -EPERM;
    }
    usleep_range(3000, 3100);
    ret = gcore_bus_write(cmd2,sizeof(cmd2));
    if (ret) {
        GTP_ERROR("write cmd2 fail");
        return -EPERM;
    }
    usleep_range(3000, 3100);
    ret = gcore_bus_write(cmd3,sizeof(cmd3));
    if (ret) {
        GTP_ERROR("write cmd3 fail");
        return -EPERM;
    }
    usleep_range(3000, 3100);
    ret = gcore_bus_write(cmd4,sizeof(cmd4));
    if (ret) {
        GTP_ERROR("write cmd4 fail");
        return -EPERM;
    }
    usleep_range(3000, 3100);
    gcore_exit_idm_mode();
...
int gcore_fw_event_notify(enum fw_event_type event)
{
    ...
    switch (event) {
        ...
        case FW_EDGE_270:
            ...
            break;
        case FW_EDGE_270_DISABLE:
            fw_event_cmd[10] = fw_event_cmd[10] & ~((0x00 | 0x01) << 7);
            notify_byte = fw_event_cmd[10];
            GTP_DEBUG("gcore fw event:edge 270 disable.");
            break;
        ...
        case FW_NFC_ENABLE:
            fw_event_cmd[8] = fw_event_cmd[8] | ((0x00 | 0x01) << 0);
            notify_byte = fw_event_cmd[8];
            GTP_DEBUG("gcore fw event:NFC enable");
            break;
        case FW_NFC_DISABLE:
            fw_event_cmd[8] = fw_event_cmd[8] & ~((0x00 | 0x01) << 0);
            notify_byte = fw_event_cmd[8];
            GTP_DEBUG("gcore fw event:NFC disable");
            break;

        case FW_GESTURE_REPORT_INT:
            GTP_DEBUG("gcore fw event:gusture report int, report gusture key.");
            fw_event_cmd[8] = fw_event_cmd[8] | ((0x00 | 0x01) << 4);
            notify_byte = ((0x00 | 0x01) << 4);
            break;
        case FW_GESTURE_REPORT_INT_CLEAR:
            GTP_DEBUG("gcore fw event:gusture report int clear.");
            fw_event_cmd[8] = fw_event_cmd[8] & ~((0x00 | 0x01) << 4);
            notify_byte = fw_event_cmd[8];
            break;

        case FW_GESTURE_SUCCESS_NOTIFY:
            GTP_DEBUG("gcore fw event:gusture click success, notify to fw.");
            fw_event_cmd[8] = fw_event_cmd[8] | ((0x00 | 0x01) << 7);
            break;
        case FW_GESTURE_SUCCESS_NOTIFY_CLEAR:
            GTP_DEBUG("gcore fw event:gusture click clear, notify to fw.");
            fw_event_cmd[8] = fw_event_cmd[8] & ~((0x00 | 0x01) << 7);
            break;
        /*HS07 code for HS07-249 by huangyin at 20250331 start*/
        case DRIVER_REGISTER_START:
            fw_event_cmd[8] = fw_event_cmd[8] | ((0x00 | 0x01) << 1);
    ...
    if (g_ret_update == 0 || gdev_fwu->ts_stat == TS_SUSPEND) {
        ...
        if(gdev_fwu->ts_stat == TS_SUSPEND && fn_data.gdev->gesture_wakeup_en){
            GTP_DEBUG("in gesture wakeup, can send cmd.");
            gdev_fwu->fw_event = event;
        }else
#endif
        {
            ...
            GTP_DEBUG("fw event:[80B0 %x %x %x %x %x %x %x %x]", \
                fw_event_cmd[3], fw_event_cmd[4], fw_event_cmd[5], fw_event_cmd[6], \
                fw_event_cmd[7], fw_event_cmd[8], fw_event_cmd[9],fw_event_cmd[10]);
            return 0;
        }
    }else{
        gdev_fwu->fw_event = event;
...
void gcore_modify_fw_event_cmd(enum fw_event_type event)
{
    ...
    GTP_DEBUG("modify event cmd[8]=%d,cmd[9]=%d,cmd[10]=%d",fw_event_cmd[8],fw_event_cmd[9],fw_event_cmd[10]);
...
{
    GTP_DEBUG("sent to fw:0x%x, receive from fw:0x%x",notify_byte,rev_data);
...
DECLARE_COMPLETION(flashdata_complete);
int flash_pos = 0;
int gcore_write_cmd_to_flashop(void)
{
#if 1
    int i = 0;
    int ret = 0;
    int retry = 0;
    int read_flash_times = 0;
    u32 addr = 0;
    //u8 cmd[] = {0x40,0x00,0xD3,0x00,(address>>8),(address&0xff),0x00,0x38};
    //u8 cmd[8] = {0x40,0x00,0xD3,0x00,0x00,0x00,0x00,0x38};
    u8 cmd[8] = {0x40,0x00,0xD3,0x03,0x00,0x00,0x00,0x38};
    flash_pos = 0;
    addr = gdev_fwu->flash_addr;
    read_flash_times = gdev_fwu->flash_data_size / GCORE_LENFTH_OF_FLASH_READ_ONCE;
    gdev_fwu->read_addr_pos = 0;
    gdev_fwu->fw_event = FW_READ_FALSH_OP;
    fw_update_fn.wait_int = true;
    cmd[1] = addr >> 16;
    cmd[4] = addr >> 8;
    cmd[5] = addr & 0xff;

    memset(gdev_fwu->flash_data_buff,0,gdev_fwu->flash_data_size);
    for(i=0;i<=read_flash_times;i++){
        do{
            init_completion(&flashdata_complete);
            ret = gcore_bus_write(cmd,sizeof(cmd));
            if(ret){
                GTP_ERROR("write cmd fail!");
                retry++;
            }else{
                ret = wait_for_completion_timeout(&flashdata_complete, msecs_to_jiffies(50));
                if (ret == 0) {
                    GTP_ERROR("wait for INT timeout!");
                    retry++;
                }else{
                    retry = 0;
                    break;
        ...
        if(retry > 0){
            GTP_DEBUG("read flash data error!");
            GTP_DEBUG("read flash end of addr:0x%x",addr);
            return -EPERM;
        }
        retry = 0;
        addr += GCORE_LENFTH_OF_FLASH_READ_ONCE;
        cmd[1] = addr >> 16;
        cmd[4] = addr >> 8;
        cmd[5] = addr & 0xff;
    }
    ...
    GTP_DEBUG("read flash end of addr:0x%x",addr);
    GTP_DEBUG("read data flash_data_buff:0x%x 0x%x 0x%x 0x%x",\
        gdev_fwu->flash_data_buff[0],gdev_fwu->flash_data_buff[1], \
        gdev_fwu->flash_data_buff[2],gdev_fwu->flash_data_buff[3]);
    fw_update_fn.wait_int = false;
#endif
    GTP_DEBUG("No use this fustion");
    return 0;

}
int gcore_flashdownload_opbin(void)
{
    int ret = 0;
    int i,retry;
    u32 addr = 0;
    u8 cmd[6] = {0x05,0xFA,0x80,0x02, 0x00, 0x00};
    u8 cmd1[7] = {0x05,0xFA,0x40,0x00,0x00,0x50,0x01};
    u8 cmd2[2] = {0x0A,0xF5};
    u8 cmd3[7] = {0x40,0xE1,0xD0,0x00,0x00,0x00,0x00};
    retry = sizeof(GC7xxx_Flash_Op) / GGCORE_FLASHDOWNLOAD_SIZE;
    for(i=0;i<retry;i++){
        cmd[4] = addr >> 8;
        cmd[5] = addr & 0xFF;
        memcpy((u8 *)gdev_fwu->fop_firmware->data, cmd, gdev_fwu->fop_cmd_len);
        memcpy((u8 *)&gdev_fwu->fop_firmware->data[gdev_fwu->fop_cmd_len], GC7xxx_Flash_Op+(i*GGCORE_FLASHDOWNLOAD_SIZE), GGCORE_FLASHDOWNLOAD_SIZE);
        ret = gcore_bus_write(gdev_fwu->fop_firmware->data,gdev_fwu->fop_firmware->size);
        if (ret) {
            GTP_ERROR("wirte flash op bin fail");
            return -EPERM;
        }
        addr+=0x400;

        GTP_DEBUG("addr=0x%x , cmd4=0x%x , cmd5=0x%x",addr,cmd[4],cmd[5]);
    }
    usleep_range(1000,1100);
    ret = gcore_bus_write(cmd1,sizeof(cmd1));
    if (ret) {
...
int gcore_read_flash_data_proc(void)
{
    int ret = 0;
    u8 cmd1[6] = {0x04,0xFB,0xC0,0x00,0x00,0x90};
    u8 cmd2[10] = {0x05,0xFA,0xC0,0x00,0x00,0x90,0x01,0x00};
    u8 read_buf[4] = {0};
    //u32 upgrade_addr = 0xC0000000 + (0x14 << 2) + 1;
    //u8 value = BIT6;    /* bit6 write 1 Upgrade */
    if(gdev_fwu->flash_data_buff == NULL){
        gdev_fwu->flash_data_buff = kzalloc(gdev_fwu->flash_data_size+1,GFP_KERNEL);
    }else{
        kfree(gdev_fwu->flash_data_buff);
        gdev_fwu->flash_data_buff = NULL;
        gdev_fwu->flash_data_buff = kzalloc(gdev_fwu->flash_data_size+1,GFP_KERNEL);
    }
    gdev_fwu->fw_update_state = true;
#if GCORE_WDT_RECOVERY_ENABLE
    //cancel_delayed_work_sync(&gdev_fwu->wdt_work);
#endif
    if (mutex_is_locked(&gdev_fwu->flashread_lock)) {
        GTP_DEBUG("touch is locked, ignore");
        return -2;
    }
    mutex_lock(&gdev_fwu->flashread_lock);
    ret = gcore_enter_idm_mode();
    if (ret) {
        GTP_ERROR("flash read enter idm mode fail");
        goto err;
    }
    usleep_range(1000,1100);
    ret = gcore_bus_write(cmd1, sizeof(cmd1));
    ...
    usleep_range(1000,1100);
    ret = gcore_bus_read(read_buf,sizeof(read_buf));
    if (ret) {
        GTP_ERROR("read buff fail");
        goto err;
    }
    usleep_range(1000,1100);
    cmd2[8] = read_buf[2] & 0xf0;
    cmd2[9] = read_buf[3];
    ret = gcore_bus_write(cmd2, sizeof(cmd1));
    if (ret) {
        GTP_ERROR("write cmd2 fail");
        goto err;
    }

    ret = gcore_flashdownload_opbin();
    if (ret) {
        GTP_ERROR("Download flash op bin fail");
        goto err;
    }
    if(!gdev_fwu->flash_data_buff){
        gdev_fwu->flash_data_buff = kzalloc(gdev_fwu->flash_data_size+1,GFP_KERNEL);
    }
    if (gdev_fwu->flash_data_buff == NULL) {
        GTP_ERROR("alloc flash data buff failed");
        goto err;
    }

    init_completion(&flashdata_complete);
    mutex_unlock(&gdev_fwu->flashread_lock);
#if 1
    if(gdev_fwu->flash_addr < 0){
        gdev_fwu->flash_addr = 0x10000;
        GTP_DEBUG("Set default flash_addr=0x%x",gdev_fwu->flash_addr);
    }
    ret = gcore_write_cmd_to_flashop();
    if (ret) {
        GTP_ERROR("write cmd to flashop fail");
        goto err;
    }
/*
    if (gcore_idm_write_reg(upgrade_addr, &value, 1)) {
        GTP_ERROR("upgrade soft reset fail");
    }
*/
    ...
    return 0;
err:
    GTP_ERROR("Get Flash data error!");
    /*
    if (gcore_idm_write_reg(upgrade_addr, &value, 1)) {
        GTP_ERROR("upgrade soft reset fail");
    }
...
void gcore_set_flashread_parameters(u32 len,u32 addr)
{
    if(len <= 0){
        GTP_DEBUG("Set Lenth error! Used default Flash lenth:%dk , addr:0x%x",gdev_fwu->flash_data_size/1024,gdev_fwu->flash_addr);
        return;
    }
    gdev_fwu->flash_data_size = len;
    gdev_fwu->flash_addr = addr;
    GTP_DEBUG("lenth:%dk addr:0x%x",gdev_fwu->flash_data_size/1024,gdev_fwu->flash_addr);

}

int gcore_fw_read_flashdata_reply(void)
{

    int ret = 0;
    if(!gdev_fwu->read_flash_buff){
        GTP_ERROR("read_flash_buff is free!");
        return -EPERM;
    }
    ret = gcore_bus_read(gdev_fwu->read_flash_buff,gdev_fwu->read_flash_len);
    if (ret) {
        GTP_ERROR("read flash buff fail ");
        return -EPERM;
    }
    memcpy(gdev_fwu->flash_data_buff+flash_pos,gdev_fwu->read_flash_buff+8,GCORE_LENFTH_OF_FLASH_READ_ONCE);
    flash_pos+=GCORE_LENFTH_OF_FLASH_READ_ONCE;
    complete(&flashdata_complete);
    //init_completion(&flashdata_complete);
    return ret;
}
...
//     g_mp_data->gdev->gcore_mpfw_rq_name,mp_data->gdev->gcore_csv_name,gdev_fwu->mp_ver_in_bin[0],gdev_fwu->mp_ver_in_bin[1], \
//     gdev_fwu->mp_ver_in_bin[2],gdev_fwu->mp_ver_in_bin[3]);
// }

#endif
extern bool data_need_decrypt_flag;
                  ...
                  const char *buf, size_t count)
{
  ...
  if (enabled) {
    ...
    if(enabled == 1){
        data_need_decrypt_flag = 1;
    }else if(enabled == 0){
        data_need_decrypt_flag = 0;
    }
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
    if(gdev_fwu->gesture_wakeup_en == 1)
        func_buf[1] = "on";
    else
        func_buf[1] = "off";
#endif
...
static ssize_t notify_event_show(struct device *dev,
                  struct device_attribute *attr, char *buf)
{
    return scnprintf(buf, PAGE_SIZE, "%s\n",
        "20250103, notify_cmd sync with fw. 1-16,17-20,33-34,41-42,47-48.");
}
                  ...
                  const char *buf, size_t count)
{
    ...
    switch (enabled)
    {
        case NOTIFY_FW_EDGE_90:
            gcore_fw_event_notify(FW_EDGE_90);
            break;
        case NOTIFY_FW_EDGE_0:
            gcore_fw_event_notify(FW_EDGE_0);
            break;
        case NOTIFY_FW_CHARGER_PLUG:
            gcore_fw_event_notify(FW_CHARGER_PLUG);
            break;
        case NOTIFY_FW_CHARGER_UNPLUG:
            gcore_fw_event_notify(FW_CHARGER_UNPLUG);
            break;
        case NOTIFY_FW_HEADSET_PLUG:
            gcore_fw_event_notify(FW_HEADSET_PLUG);
            break;
        case NOTIFY_FW_HEADSET_UNPLUG:
            gcore_fw_event_notify(FW_HEADSET_UNPLUG);
            break;
        case NOTIFY_FW_GESTURE_ENABLE:
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
            set_gcore_gesture_en(1);
#else
            gcore_fw_event_notify(FW_GESTURE_ENABLE);
#endif
            break;
        case NOTIFY_FW_GESTURE_DISABLE:
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
            set_gcore_gesture_en(0);
#else
            gcore_fw_event_notify(FW_GESTURE_DISABLE);
#endif
            break;
        case NOTIFY_FW_GLOVE_ENABLE:
            gcore_fw_event_notify(FW_GLOVE_ENABLE);
            break;
        case NOTIFY_FW_GLOVE_DISABLE:
            gcore_fw_event_notify(FW_GLOVE_DISABLE);
            break;
        case NOTIFY_FW_REPORT_RATE_180:
            gcore_fw_event_notify(FW_REPORT_RATE_180);
            break;
        case NOTIFY_FW_REPORT_RATE_120:
            gcore_fw_event_notify(FW_REPORT_RATE_120);
            break;
        case NOTIFY_FW_REPORT_HIGH_SENSITIVITY:
            gcore_fw_event_notify(FW_REPORT_HIGH_SENSITIVITY);
            break;
        case NOTIFY_FW_REPORT_NORMAL_SENSITIVITY:
            gcore_fw_event_notify(FW_REPORT_NORMAL_SENSITIVITY);
            break;
        case NOTIFY_FW_EDGE_270_ENABLE:
            gcore_fw_event_notify(FW_EDGE_270);
            break;
        case NOTIFY_FW_EDGE_270_DISABLE:
            gcore_fw_event_notify(FW_EDGE_270_DISABLE);
            break;

        case NOTIFY_FW_TEL_CALL:
#ifdef CONFIG_ENABLE_PROXIMITY_TP_SCREEN_OFF
            tpd_enable_ps(1);
#else
...
#endif
            break;
        case NOTIFY_FW_HANDSFREE_ON:
            gcore_fw_event_notify(FW_HANDSFREE_ON);
            break;
        case NOTIFY_FW_HANDSFREE_OFF:
            gcore_fw_event_notify(FW_HANDSFREE_OFF);
            break;

        case NOTIFY_FW_NFC_ENABLE:
            gcore_fw_event_notify(FW_NFC_ENABLE);
            break;
        case NOTIFY_FW_NFC_DISABLE:
            gcore_fw_event_notify(FW_NFC_DISABLE);
            break;

        case NOTIFY_FW_GESTURE_REPORT_INT:
            gcore_fw_event_notify(FW_GESTURE_REPORT_INT);
            break;
        case NOTIFY_FW_GESTURE_REPORT_INT_CLEAR:
            gcore_fw_event_notify(FW_GESTURE_REPORT_INT_CLEAR);
            break;

        case NOTIFY_FW_GESTURE_SUCCESS_NOTIFY:
            gcore_fw_event_notify(FW_GESTURE_SUCCESS_NOTIFY);
            break;
        case NOTIFY_FW_GESTURE_SUCCESS_NOTIFY_CLEAR:
            gcore_fw_event_notify(FW_GESTURE_SUCCESS_NOTIFY_CLEAR);
            break;

#ifdef CONFIG_ENABLE_PROXIMITY_TP_SCREEN_OFF
        case NOTIFY_PROXIMITY_NEAR:
        ...
        gcore_tpd_proximity_flag_off = true;
            ...
            GTP_DEBUG("proximity near,ps_report 0.");
    #else
            input_report_abs(gdev_intf->input_ps_device, ABS_DISTANCE, 0);
            input_sync(gdev_intf->input_ps_device);
            GTP_DEBUG("proximity near,input_report ABS_DISTANCE 0.");
    #endif
        break;

        case NOTIFY_PROXIMITY_AWAY:
        ...
        gcore_tpd_proximity_flag_off = false;
            ...
            GTP_DEBUG("proximity away,ps_report 10.");
    #else
            input_report_abs(gdev_intf->input_ps_device, ABS_DISTANCE, 1);
            input_sync(gdev_intf->input_ps_device);
            GTP_DEBUG("proximity away,input_report ABS_DISTANCE 1.");
    #endif
        break;
...
static DEVICE_ATTR(notify_fw, 0600, notify_event_show, notify_event_store);
...
static int fwu_event_handler(void *p)
{
    ...
    do {
        ...
        case FW_READ_DIFFDATA:
            ...
            break;
#ifdef CONFIG_GET_DATA_FROM_FLASH
        case FW_READ_FALSH_OP:
            gcore_fw_read_flashdata_reply();
            break;
#endif
        default:
