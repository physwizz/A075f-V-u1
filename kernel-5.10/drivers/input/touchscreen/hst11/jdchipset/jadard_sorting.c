...
static int jd_test_data_pop_out(char *rslt_buf, char *filepath, int result)
{
    ...
    if (result == 0) {
        scnprintf(filepath, JD_RSLT_OUT_FILE_SIZE, "%s_Pass.txt", filepath);
    } else {
        scnprintf(filepath, JD_RSLT_OUT_FILE_SIZE, "%s_Fail.txt", filepath);
    }
...
static int jadard_parse_sorting_threshold_file(void)
{
    ...
    char *file_name = "jd9366tc_xy_csot_ito.txt";
#else
...
static int jadard_sorting_test(void)
{
    ...
    uint32_t fw_ret = 0;
    char *fileName = "jd9366tc_xy_csot_firmware.bin";
    ...
    scnprintf(jd_g_file_path, JD_RSLT_OUT_FILE_SIZE, "%s%s_%s%s.txt",
         JD_RSLT_OUT_PATH, ret == 0 ? "PASS" : "FAIL", JD_RSLT_OUT_FILE, get_date_time_str());
...
#ifdef JD_UPGRADE_ITO_FW
    fileName = "jd9366tc_xy_csot_firmware.bin";
    JD_I("%s: Bin file name(%s)\n", __func__, fileName);
