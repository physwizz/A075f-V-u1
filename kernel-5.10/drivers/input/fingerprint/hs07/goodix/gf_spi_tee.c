...
void gf_cleanup(struct gf_device *gf_dev)
{
    ...
    if (IS_ERR_OR_NULL(gf_dev->pinctrl_gpios)) {
        gf_debug(ERR_LOG, "%s pinctrl_gpios is null, can not put", __func__);
    } else {
...
static int gf_get_pinctrl_dts_info(struct gf_device *gf_dev)
{
    ...
    node = of_find_compatible_node(NULL, NULL, "mediatek,goodix-platform-fp");
    if (node) {
    ...
    gf_dev->pins_reset_high = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "gf_reset_high");
    ...
    gf_dev->pins_reset_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "gf_reset_low");
    ...
    gf_dev->pins_vdd_high = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "gf_vdd_high");
    ...
    gf_dev->pins_vdd_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "gf_vdd_low");
    ...
    gf_dev->pinstate_gpio_func = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_gpio_mode");
    if (IS_ERR_OR_NULL(gf_dev->pinstate_gpio_func)) {
        ret = PTR_ERR(gf_dev->pinstate_gpio_func);
        gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl fp_gpio_mode\n", __func__);
        devm_pinctrl_put(gf_dev->pinctrl_gpios);
        gf_dev->pinctrl_gpios = NULL;
        gf_dev->pins_reset_high = NULL;
        gf_dev->pins_reset_low = NULL;
    ...
    gf_dev->pinstate_spi_func = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_spi_mode");
    if (IS_ERR_OR_NULL(gf_dev->pinstate_spi_func)) {
        ret = PTR_ERR(gf_dev->pinstate_spi_func);
        gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl fp_spi_mode\n", __func__);
        devm_pinctrl_put(gf_dev->pinctrl_gpios);
        gf_dev->pinctrl_gpios = NULL;
        gf_dev->pins_reset_high = NULL;
        gf_dev->pins_reset_low = NULL;
    ...
    /*HS07 code for SR-AL7761A-01-184 by hehaoran at 20250325 start*/
    gf_dev->pinstate_irq_to_gpio = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "gf_irq_to_gpio");
    if (IS_ERR_OR_NULL(gf_dev->pinstate_irq_to_gpio)) {
        ret = PTR_ERR(gf_dev->pinstate_irq_to_gpio);
        gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl gf_irq_to_gpio\n", __func__);
        devm_pinctrl_put(gf_dev->pinctrl_gpios);
        gf_dev->pinctrl_gpios = NULL;
        gf_dev->pins_reset_high = NULL;
        gf_dev->pins_reset_low = NULL;
    ...
    if (gf_dev->pinstate_gpio_func) {
        pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pinstate_gpio_func);
        if (retval < 0){
            gf_debug(ERR_LOG, "%s:set fp_gpio_mode failed,retval=%d", __func__, retval);
        }
    }
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
    ...
    if (IS_ERR_OR_NULL(gf_dev->pinctrl_gpios)) {
        gf_debug(ERR_LOG, "%s:pinctrl_gpios is null, cant set hw_power", __func__);
        return;
    }
    ...
    if (onoff) {
        ...
        if(gf_dev->pins_vdd_high) {
            retval = pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_vdd_high);
            if (retval < 0){
                gf_debug(ERR_LOG, "%s:set pins_vdd_high failed,retval=%d", __func__, retval);
            } else {
                gf_debug(INFO_LOG, "%s:set pins_vdd_high success", __func__);
            }
        }
        ...
        if (gf_dev->pinstate_spi_func) {
            ...
            if (retval < 0){
                gf_debug(ERR_LOG, "%s:set pinstate_spi_func failed,retval=%d", __func__, retval);
            }
        }
        ...
        if (gf_dev->pinstate_gpio_func) {
            pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pinstate_gpio_func);
            if (retval < 0){
                gf_debug(ERR_LOG, "%s:set pinstate_gpio_func failed,retval=%d", __func__, retval);
            }
        }
        /*HS07 code for SR-AL7761A-01-184 by hehaoran at 20250325 start*/
        mdelay(5);
        ...
        if(gf_dev->pins_vdd_low) {
            retval = pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_vdd_low);
            if (retval < 0){
                gf_debug(ERR_LOG, "%s:set pins_vdd_low failed,retval=%d", __func__, retval);
            } else {
                gf_debug(INFO_LOG, "%s:set pins_vdd_low success", __func__);
            }
        }
        /*HS07 code for HS07-1471 by hehaoran at 20250424 end*/
    }
...
#ifdef CONFIG_OF
    if(!IS_ERR_OR_NULL(gf_dev->pins_reset_high)) {
        retval = pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_reset_high);
        if (retval < 0){
            gf_debug(ERR_LOG, "%s:set pins_reset_high failed,retval=%d", __func__, retval);
        }
    }
#endif
}
...
static void gf_hw_reset(struct gf_device *gf_dev, u8 delay)
{
    ...
    if (IS_ERR_OR_NULL(gf_dev->pinctrl_gpios)) {
        gf_debug(ERR_LOG, "%s:pinctrl_gpios is null, cant set hw_reset", __func__);
        return;
    }

    if(gf_dev->pins_reset_low) {
        retval = pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_reset_low);
        if (retval < 0){
            gf_debug(ERR_LOG, "%s:set pins_reset_low failed,retval=%d", __func__, retval);
        }
    }
    /*HS07 code for HS07-1471 by hehaoran at 20250424 start*/
    mdelay(12);
...
static void gf_irq_to_gpio_mode(struct gf_device *gf_dev)
{
    ...
    if(gf_dev->pinstate_irq_to_gpio) {
        retval = pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pinstate_irq_to_gpio);
        if (retval < 0) {
            gf_debug(ERR_LOG, "%s:set pinstate_irq_to_gpio failed,retval=%d", __func__, retval);
        }
    }
    mdelay(3);
    FUNC_EXIT();
    ...
    */
    ...
    switch (cmd) {
        case GF_IOC_INIT:
            ...
            retval = request_threaded_irq(gf_dev->irq, NULL, gf_irq, IRQF_TRIGGER_RISING | IRQF_ONESHOT, "goodix_fp_irq", gf_dev);
            if (!retval) {
        ...
        case GF_IOC_INPUT_KEY_EVENT:
            ...
            gf_debug(INFO_LOG, "%s: GF_IOC_INPUT_KEY_EVENT, not support input key\n", __func__);
#endif//GF_NAV_INPUT_SUPPORT
            break;
...
static int gf_open(struct inode *inode, struct file *filp)
{
    ...
    if (gf_dev->system_status) {
        ...
        if (status < 0){
            gf_debug(ERR_LOG, "%s,fail gf_get_pinctrl_dts_info", __func__);
        }
    }
